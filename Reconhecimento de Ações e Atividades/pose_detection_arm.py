...
def detect_pose_and_count_arm_movements(video_path, output_path):
    ...
    heart_drawing = False
    heart_movements_count = 0

    # Função para verificar se o braço está levantado
    def is_heart(landmarks):
        left_shoulder = landmarks[mp.tasks.vision.PoseLandmark.LEFT_SHOULDER.value]
        right_shoulder = landmarks[mp.tasks.vision.PoseLandmark.RIGHT_SHOULDER.value]
        left_wrist = landmarks[mp.tasks.vision.PoseLandmark.LEFT_WRIST.value]
        right_wrist = landmarks[mp.tasks.vision.PoseLandmark.RIGHT_WRIST.value]

        left_heart_arm_draw = left_wrist.x < left_shoulder.x
        right_heart_arm_draw = right_wrist.x > right_shoulder.x

        return left_heart_arm_draw and right_heart_arm_draw
    ...
    for frame_idx in tqdm(range(total_frames), desc="Processando vídeo"):
        ...
        if result.pose_landmarks:
            ...
            if is_heart(pose_landmarks):
                if not heart_drawing:
                    heart_drawing = True
                    heart_movements_count += 1
            else:
                heart_drawing = False

        # Exibir a contagem de movimentos dos braços no frame
        cv2.putText(frame, f'Desenhos de coracao: {heart_movements_count}', (10, 30),
